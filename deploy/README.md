...
Нет сети — скрипт выходит с кодом 75 (`EX_TEMPFAIL`): мак в дороге это норма,
следующий круг догонит. Пропуск не успех и не поломка — штамп свежести не
двигается, но и жаловаться не на что, интерфейс просто покажет растущее
«обновлено N мин назад». Ноль здесь был бы враньём: приложение засчитывало бы
пропуск синхронизацией и писало «Данные свежие» поверх цифр любой давности.
Отказ сервера завершается другим ненулевым кодом, и причина видна в интерфейсе
строкой состояния.
