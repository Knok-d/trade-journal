...
Отдельного бандла нет и не нужно: Safari → открыть `http://127.0.0.1:8321` →
меню «Поделиться» → «Добавить в Dock». Получается окно без адресной строки со
своей иконкой в Dock и Launchpad. Приложение уже поднято агентом, так что окно
открывается сразу.
